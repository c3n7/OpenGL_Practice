...
**Note**
- It is also possible to omit the `layout (location = 0)` specifier and query for the attribute llocations in your OpenGl code via `glGetAttribLocation`, but setting them in the vertex shader makes it easier to understand and saves you (and OpenGL) some work.  
**End Note**
...
- Since uniforms are global variables, we can define them in any shader we'd like so ni need to go through the vertex shader again to get something to the fragment shader.
...
float greenValue = (sin(timmeValue) / 2.0f) + 0.5f;
...
- Uniforms are useful but when we want change the color of each vertex, we'd have to declare as many unifroms as we have vertices.
